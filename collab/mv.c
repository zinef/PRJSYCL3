...
void mv_normal (char source[100], char destination[100]){
...
int fd1,fd2,nbread;
 ...
 //1-on verifie qu'on a recu deux arguments 
   ...
   //2- on vérifie si la source= destination le fichier ne sera pas copier comme  on aura juste une duplication. 
     //2.1- on récupere le chemin ou il devra etre a la destination
        char chemin[100]="";  
        char *nomf;
  ...
  //3- si les parametres sont bien entrés et different on ouvre le fichier source pour vérfier qu'il est bien présent 
     fd1 = open(source,O_RDONLY);
     if (fd1<0){
       if (errno== ENOENT){
          printf("\nLe fichier que vous avez introduit n'est pas valide \n");
          exit(EXIT_FAILURE);
        }
    ...
    //4-on teste si le chemin destination est valide avec la valeur de retour de chdr 
   strcpy(origine, "");
   getcwd( origine, sizeof(origine));   // on sauvgarde le chemin actuel
   int continu=0;
    continu=chdir(destination);
   if (continu ==-1)
      {
      printf("\nLe chemin destination n'est pas valide veuillez le vérifier\n");
      exit(EXIT_FAILURE);
      }
     chdir(origine);
   //5- On effectue la commande :
    rename(source,chemin);
   close(fd1);close(fd2);
...
void mp_source_tar(char source[100], char destination[100]){
...
void mv_destination_tar(char source[100], char destination[100]) { 
int fd1,fd2,errno,nbread,nblus,nbtotal=0;
int fd,i,j,n,m;
int erreur=0;
...
int size=0;char origine[100];
	...
	if(strcmp(suite,"") != 0){
        ...
        // maintenant dans nomfic, on a le chemin apartir du tar vers le fichier a copier 
	...
	printf("Le fichier source %s existe déja dans la destination",nomf);
	exit(EXIT_FAILURE); 
	}else{
...
void mv_source_destination_tar(char source[100], char destination[100]) { 
int fd1,fd2,errno,nbread,nblus,nbtotal=0;
int fd,i,j,n,tmp,m;
int erreur=0;
...
int mv(char source[100], char destination[100])
{
...
strcpy(str2,destination);
  ...
  if ((strstr(str1, ".tar/")== NULL)&& (strstr(str2, ".tar/")== NULL)){ 
      //  Fonctionnement normal de mv /
       mv_normal(source,destination);
       exit(EXIT_FAILURE);
  ...
  if ((strstr(source, ".tar/")!= NULL)&& (strstr(destination, ".tar/")== NULL)){
       ...
       // mv_source_tar(source,destination);
        exit(EXIT_FAILURE);
      }
       
  if ((strstr(source, ".tar/")== NULL)&& (strstr(destination, ".tar/")!= NULL)){
       // 1.2- source comporte pas tar --> destination  tar
       mv_destination_tar(source,destination);
       exit(EXIT_FAILURE);
      }   
  ...
  if ((strstr(source, ".tar/")!= NULL)&& (strstr(destination, ".tar/")!= NULL)){
      ...
      //mv_source_destination_tar(source,destination);
       exit(EXIT_FAILURE);
      }
  }
}
...
int main(){
mv_source_tar("/home/faycal/Documents/ProjetSyst/f1.txt", "/home/faycal/Documents/ProjetSyst/test.tar/");
