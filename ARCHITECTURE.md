...
### Choix concernant l'implémentation de quelques fonctionnalités
+ Pour la suppression dans les Tarballs , on a pris le choix d'implémenter une suppression logique des fichiers qui n'engendre pas une grande complexité algorithmique  , notamment si on fait des décalages de BLOCK dans le fichier ça engendre un coût énorme en terme d'exécution si le fichier Tar est de grande taille ,  ainsi pour une suppression logique il faut évidemment fixer des règles pour que tout soit cohérent (pour toutes les commandes implémentées en générale et pour les fonctions en particulier).
	+ **Choix d'un champs d'entête à modifier** : pour cela on a choisi de modifier le champs `name` de la structure `posix_header` , et exactement de modifier le premier caractère pour un `#` ,notamment que les fichiers ou répertoires ne doivent pas contenir des caractères spéciaux , et pour les champs qui suivent ne seront pas touchés , notamment le champs `size` qui est important pour le déplacement entre entêtes dans le Tar , donc même si le fichier est supprimé "logiquement" (i.e. d'une manière logique) on va pouvoir ajouter d'autres sans problèmes avec les déplacements . 
	+ **Ajout de fichiers ou répertoires** : comme le champs d'entête à modifier à priori est définie comme `(st->name[0] == '#')` donc au moment de l'ajout on va profiter le fait qu'un fichier (régulier ou répertoire ) est supprimé logiquement pour ajouter le nouveau à ça place (utilisation de `mkdir` par exemple : si le répertoire à ajouter ne figure pas dans le répertoire actuel et qu'il existe un répertoire supprimé "logiquement" donc le nouveau répertoire prendra la place de celui supprimé on modifiant cette fois ci bien sur les caractéristiques pour qu'elle correspondent bien au répertoire ajouté , de même pour `rm -r` ) , de même pour les commandes  `cp` et `mv`  quand leurs chemins impliquent des Tarballs.
	+ **Au moment d'affichage** : évidement si un fichier commence par un `#` celui ci ne doit pas être affiché à l'utilisateur qui tapera par exemple la commande `ls` ou `ls -l` .  
